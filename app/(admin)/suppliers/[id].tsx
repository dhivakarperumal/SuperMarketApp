...
import { ChevronLeft, Building2, Trash2 } from "lucide-react-native";
...
export default function SupplierDetailScreen() {
  ...
  const { suppliers, updateSupplier, deleteSupplier, loading: suppliersLoading } = useSuppliers();
  ...
  // Load supplier data
  useEffect(() => {
    if (!suppliersLoading && id && !initialized) {
  ...
  }, [suppliers, id, suppliersLoading, initialized]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Toast.show({ type: "error", text1: "Error", text2: error.message || "Failed to update supplier" });
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      Toast.show({ type: "error", text1: "Error", text2: error.message || "Failed to delete supplier" });
    }
  };
    ...
    <SafeAreaView className="flex-1 bg-[#F1F8E9]" edges={["top","bottom"]}>
      ...
      <View className="flex-row items-center justify-between px-4 py-4 bg-white border-b border-gray-100">
        <View className="flex-row items-center">
          ...
          <Text className="text-xl font-bold text-gray-800">Edit Supplier</Text>
        </View>
        <Pressable onPress={handleDelete} className="p-2.5 bg-red-50 rounded-full">
